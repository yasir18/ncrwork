...
int main() {
	...
	FILE *fp,*fp2,*fp3,*fp4;
	 fopen_s(&fp , "file.txt", "r");//fp to perform count operations
	 fopen_s(&fp2, "write.txt", "w");//fp to remove comments from a file
	 fopen_s(&fp3, "rotate.txt", "w");//fp to find and replace word with its rotated word
	 fopen_s(&fp4, "replacetabs.txt", "w");//fp to replace tabs with \t
	scanf_s("%d", &choice);
	switch(choice){
		case 1:
			countt(fp);
